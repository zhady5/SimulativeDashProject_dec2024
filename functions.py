...
def get_current_previous_sums(df, col, period):
    mask1 = (df.data.apply(lambda d: convert_date(d, '%Y-%m-%d')) <= convert_date(date_ago(period[0]), '%Y-%m-%d').date())
    mask2 = (df.data.apply(lambda d: convert_date(d, '%Y-%m-%d')) > convert_date(date_ago(period[1], period[2]), '%Y-%m-%d').date())
    mask3 = (df.data.apply(lambda d: convert_date(d, '%Y-%m-%d')) <= convert_date(date_ago(period[1], period[2]), '%Y-%m-%d').date())
    mask4 = (df.data.apply(lambda d: convert_date(d, '%Y-%m-%d')) > convert_date(date_ago(period[1], period[2]*2), '%Y-%m-%d').date())
